...
# --- ARGUMENT PARSING ---
parser = argparse.ArgumentParser(description="Unified Pipeline for Experiment 3 (3a, 3b, full)")
parser.add_argument('--data_id', type=str, required=True, help="ID of the simulation data configuration")
parser.add_argument('--mode', type=str, choices=['3a', '3b', 'full'], required=True, 
                    help="3a: Data varies, 3b: Model varies, full: Both vary")
...
# --- CUSTOM IMPORTS ---
from src.utils.experiments_run_utils import split_list_in_chunks, make_experiment_3
from src.utils.simulations_utils import generate_simulation

# Importamos la configuración específica del Experimento 3
...
def main():
    logging.info(f"▶️ STARTING EXPERIMENT 3 - MODE: {MODE} | DATA_ID: {DATA_ID}")
    
    if DATA_ID not in CONFIG_EXPERIMENT:
        logging.error(f"DATA_ID '{DATA_ID}' not found in configuration for Experiment 3.")
        sys.exit(1)
    ...
    if not is_simulation:
        ...
        try:
            ...
            # Actualizamos config con metadatos si es necesario
    ...
    for chunk_id, chunk_seeds in enumerate(tqdm(all_chunks, desc=f'Mode {MODE} Chunks')):
        ...
        # --- LÓGICA DE SALTO DE CHUNK COMPLETO ---
        if os.path.exists(chunk_path) and not FORCE_EXECUTION:
            try:
                ...
                if all(seed in chunk_results for seed in chunk_seeds):
                    logging.info(f"⏭️  Chunk {chunk_id}: Already complete. Skipping.")
                    continue
                else:
                    logging.info(f"📦 Chunk {chunk_id}: Partial found. Resuming.")
            except Exception as e:
                logging.error(f"Error loading chunk {chunk_id}: {e}")
                chunk_results = {}

        # 5.1 Recuperar de Global
        ...
        for rs_loop in chunk_seeds:
            ...
            try:
                ...
                # Llamada específica al Experimento 3
                new_results = make_experiment_3(**experiment_config, X=X, y=y, random_state=model_seed)
                chunk_results[rs_loop] = new_results
                chunk_needs_save = True
            except Exception as e:
                logging.error(f"Error in Exp 3, seed {rs_loop}: {e}")
                continue
    ...
    # =========================================================================
    logging.info("STEP 5: Consolidating final results for Exp 3...")
